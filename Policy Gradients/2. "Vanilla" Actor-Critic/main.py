...
def main():
	...
	actor_critic_agent = ActorCritic_agent(env, pg_hidden_size, pg_lr, pg_batch_size,\
	...
	for episode_num in range(0, pg_total_episodes):
		...
		while not done:
			# pick an action
			action = actor_critic_agent.act(state)            
			# act and get feedback from the environment
			new_state, reward, done, info = env.step(action)

			# add the transition to the experience buffer
			actor_critic_agent.memorize(state, action, new_state, reward, done)

			rewards.append(reward)
			states.append(state)
			actions.append(action)

			new_action = actor_critic_agent.act(new_state)
		...
		for reward, state, action in zip(rewards, states, actions):
			if current_pg_batch_size == pg_batch_size:
				...
				actor_critic_agent.learn(rewards_batch, states_batch, actions_batch,\
